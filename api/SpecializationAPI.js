const SpecializationRepository = require('../repository/sequelize/SpecializationRepository');
...
exports.getSpecializationById = (req, res, next) => {
    ...
    SpecializationRepository.getSpecializationById(specializationId)
        .then(specialization => {
            if (!specialization) {
                res.status(404).json({
...
exports.createSpecialization = (req, res, next) => {
    SpecializationRepository.createSpecialization(req.body)
        .then(newObj => {
...
exports.updateSpecialization = (req, res, next) => {
    const specializationId = req.params.specializationId;
    SpecializationRepository.updateSpecialization(specializationId, req.body)
        .then(result => {
...
exports.deleteSpecialization = (req, res, next) => {
    ...
    SpecializationRepository.deleteSpecialization(specializationId)
        .then(result => {
            res.status(200).json({message: 'Removed specialization', specialization: result});
        })
        .catch(err => {
            if (!err.statusCode) {
                err.statusCode = 500;
...
exports.deleteManySpecializations = (req, res, next) => {
    const specializationIds = req.body.specializationIds;
    SpecializationRepository.deleteManySpecializations(specializationIds)
        .then(result => {
            res.status(200).json({message: 'Removed specializations', specialization: result});
        })
        .catch(err => {
            if (!err.statusCode) {
                err.statusCode = 500;
